...
**No secrets in repository**: Credentials are cached in `~/.cache/mcp-remote-testing/credentials.json` (user's home directory, not in repo).
...
- Location: `~/.cache/mcp-remote-testing/credentials.json`
...
- Review credential cache: `~/.cache/mcp-remote-testing/credentials.json`
