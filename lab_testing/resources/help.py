...
def get_help_content() -> Dict[str, Any]:
    ...
    return {
        ...
        "quick_start": {
            ...
            "standard_vpn_setup": [
                "✅ **Standard WireGuard VPN is working well** - Use this for hardware lab access",
            ...
            "foundries_vpn_setup": [
                "✅ **CURRENT STATUS: Foundries VPN is working!**",
                "",
                "**What's Working:**",
                "  ✅ Client VPN connection (after peer registration)",
                "  ✅ NetworkManager routing configuration (only routes VPN network, preserves internet)",
                "  ✅ VPN status checking and connection tools",
                "  ✅ Device listing via FoundriesFactory API",
                "  ✅ Enable/disable VPN on devices",
                "  ✅ Server-side `factory-wireguard-server` daemon running",
                "  ✅ Devices with VPN enabled automatically added as peers",
                "  ✅ Device-to-device communication enabled (via `enable_foundries_device_to_device` tool)",
                "",
                "**CRITICAL LEARNING - Device-to-Device Communication:**",
                "  ⚠️  Foundries devices default to restrictive `allowed-ips=10.42.42.1/32` (server IP only)",
                ...
                "9. Verify: 'verify_foundries_vpn_connection()' to test VPN connectivity",
                "10. Comprehensive validation: 'validate_foundries_device_connectivity(device_name?)' for step-by-step validation",
                "",
                "**Manual Method:**",
        ...
        "tools": {
            ...
            "foundries_vpn_management": {
                ...
                "validate_foundries_device_connectivity": "Comprehensive step-by-step validation of Foundries VPN and device connectivity. Performs validation in sequence: 1) Check connection to Foundries VPN server (ping test), 2) List relevant Foundries devices, 3) Check devices are online and VPN is enabled, 4) Test ping and SSH connectivity to devices. Provides clear, sequential validation results to help diagnose connectivity issues. Optional: device_name (validates specific device) or factory (factory name). Use this to systematically validate your connection setup.",
                "connect_foundries_vpn": "Connect to Foundries VPN server. Requires a WireGuard configuration file obtained from FoundriesFactory. Searches for config in standard locations if not provided. Requires fioctl CLI tool. ⚠️ CRITICAL: Your client must be registered as a peer on the server before connecting. Contact ajlennon@dynamicdevices.co.uk for assistance.",
        ...
        "common_workflows": {
            ...
            "file_transfer_decision_tree": [
                ...
                "  - Single file → copy_file_to_device",
                "  - Multiple files (<10) → copy_files_to_device_parallel (fastest)",
                "  - Large directory with rsync available → sync_directory_to_device (best for many files)",
                "  - Large directory without rsync → copy_files_to_device_parallel (fallback)",
                "  - Download file from device → copy_file_from_device",
                "  - Deploy application (binaries + configs) → copy_files_to_device_parallel",
                "  - Sync project directory → sync_directory_to_device (if rsync available) or copy_files_to_device_parallel",
            ],
            ...
            "foundries_vpn_workflow": [
                ...
                "⚠️  Before you can connect, your client must be registered as a peer on the Foundries WireGuard server.",
        ...
        "troubleshooting": {
            ...
            "foundries_vpn_not_connecting": [
                ...
                "⚠️  Your client must be added as a peer on the Foundries WireGuard server before connecting.",
                "",
                "**Check Registration:**",
                ...
                "",
                "**Server Status:**",
                "  ✅ `factory-wireguard-server` daemon is running",
                "  ✅ Devices with VPN enabled are automatically added as peers",
                "  ✅ Client peers managed via /etc/wireguard/factory-clients.conf (Priority 2 implemented)",
                "  ⚠️  Verify `/etc/hosts` is populated on server (check server if hostnames don't resolve)",
                "",
        ...
        "learnings": {
            "vpn_architecture": {
                "standard_vpn": "✅ Standard WireGuard VPN: Direct peer-to-peer connection to hardware lab network. Simple, reliable, works well. Uses NetworkManager routing to only route target networks, preserving internet connectivity.",
                "foundries_vpn": "✅ Foundries VPN: Server-based architecture with centralized VPN server on Proxmox. Client connects to server, server daemon (`factory-wireguard-server`) monitors FoundriesFactory API and automatically manages device peers. More complex but enables automatic device discovery and management.",
            ...
            "routing_configuration": {
                "critical_issue": "⚠️ CRITICAL: Without proper routing configuration, VPN becomes default route and breaks internet connectivity.",
            ...
            "server_daemon": {
                ...
                "status": "✅ Daemon is now running and working correctly. Devices with VPN enabled are automatically added as peers.",
            ...
            "client_tools": {
                "status": "✅ All Foundries VPN client-side tools work correctly: connection, status checking, device listing, enable/disable VPN on devices.",
        ...
        "best_practices": [
            ...
            "  - Chain operations based on next_steps: e.g., cache_credentials → install_ssh_key → enable_passwordless_sudo",
