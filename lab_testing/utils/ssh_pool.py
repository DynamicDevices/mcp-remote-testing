...
def get_persistent_ssh_connection(device_ip: str, username: str, device_id: str, ssh_port: int = 22) -> Optional[subprocess.Popen]:
    """
    ...
    with _pool_lock:
        if len(_connection_pool) >= MAX_POOL_SIZE:
            logger.warning(f"Connection pool full ({MAX_POOL_SIZE}), cleaning up oldest connections")
    ...
    try:
        ...
        ssh_cmd = [
            ...
            "-o", "ControlPersist=300",  # Keep master alive for 5 minutes
            "-o", "BatchMode=yes",
...
def execute_via_pool(device_ip: str, username: str, command: str, device_id: str, ssh_port: int = 22) -> subprocess.CompletedProcess:
    """
